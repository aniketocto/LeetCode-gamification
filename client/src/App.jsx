...
export default function App() {
  ...
  const [token, setToken] = useState(localStorage.getItem('lc_rpg_token') || null);
  ...
  const fetchGameData = async (authToken) => {
    try {
      ...
      const probRes = await fetch(`${API_BASE_URL}/progression/solved`, { headers });
  ...
  const handleSyncProfile = async () => {
    ...
    try {
      ...
      if (data.xpGained > 0) {
        ...
        if (data.levelUp) {
          setLevelUpData({ level: data.currentLevel, rank: data.syncedProblems[0]?.title || 'ALGORITHM SCOUT' });
  ...
  const handleResetProgress = async () => {
    if (window.confirm('⚠️ WARNING: This will permanently wipe all completed quests, XP, level, and LeetCode link history. Do you wish to proceed?')) {
      try {
  ...
  const filteredQuests = quests.filter(q => {
    ...
    const matchesCategory = filters.category === 'ALL' || (q.topics && q.topics.includes(filters.category));
  ...
  if (!token) {
    return (
      <div className="app-shell" style={{ maxWidth: '480px', marginTop: '40px' }}>
        ...
        <div className="dashboard-card pixel-border">
          ...
          <div className="card-content">
            <form onSubmit={handleAuthSubmit}>
              ...
              {authError && (
                <div style={{ color: '#ff003c', fontFamily: 'Share Tech Mono', marginBottom: '12px', fontSize: '14px' }}>
                  ❌ ERROR: {authError.toUpperCase()}
                </div>
              )}

              <button type="submit" className="pixel-btn" style={{ width: '100%', fontSize: '12px', padding: '10px' }}>
                {authMode === 'login' ? 'ENTER THE ARCHIVES' : 'START QUEST JOURNEY'}
              </button>
            </form>

            <hr className="pixel-divider" style={{ margin: '16px 0' }} />

            <div style={{ textAlign: 'center', fontSize: '13px', fontFamily: 'Share Tech Mono' }}>
              {authMode === 'login' ? (
                <>
                  New adventurer?{' '}
                  <span 
    ...
    <div className="app-shell">
      ...
      <header className="app-header pixel-border">
        ...
        <div className="header-controls">
          <button onClick={handleSoundToggle} className="sound-btn pixel-btn" aria-label="Toggle Sound">
              ...
              <div className="xp-bar-wrapper">
                <div className="xp-labels">
                  ...
                  <span className="tech-font">
                    {progression ? `${progression.xpEarnedInLevel} / ${progression.xpRequiredForNextLevel} XP` : '0 / 50 XP'}
                  </span>
                </div>
            ...
            <div className="card-content stats-list">
                ...
                <div className="stat-value-group">
                  ...
                  <span className="tech-font stat-subtext">
                    ({progression && progression.stats.grandTotal > 0 
                      ? Math.round((progression.stats.totalCleared / progression.stats.grandTotal) * 100) 
                      : 0}%)
                  </span>
                </div>
              </div>
              <div className="pixel-progress-bar min-bar">
                <div 
                  ...
                  style={{ 
                    width: `${progression && progression.stats.grandTotal > 0 
                      ? (progression.stats.totalCleared / progression.stats.grandTotal) * 100 
                      : 0}%` 
                  }}
                ></div>
              </div>
              ...
              <div className="stat-item streak-item">
                ...
                <span className={`tech-font ${progression?.streak > 0 ? 'active-streak' : ''}`}>
                  🔥 {progression?.streak || 0} DAY{progression?.streak === 1 ? '' : 'S'}
                </span>
              </div>
          ...
          <div className="dashboard-card pixel-border">
            ...
            <div className="card-content">
              {user?.leetcode?.username ? (
                <div>
                  <div style={{ fontFamily: 'Share Tech Mono', fontSize: '14px', marginBottom: '8px' }}>
                    LINKED ID: <span style={{ color: 'var(--color-primary)', fontWeight: 'bold' }}>{user.leetcode.username}</span>
                  </div>
                  <button
                    ref={syncBtnRef}
                  ...
                  {user.leetcode.lastSyncedAt && (
                    <div style={{ fontFamily: 'Share Tech Mono', fontSize: '11px', color: 'var(--color-text-gray)', textAlign: 'center' }}>
                      LAST SYNC: {new Date(user.leetcode.lastSyncedAt).toLocaleTimeString()}
                    </div>
                  )}
                </div>
              ) : (
          ...
          <div className="dashboard-card pixel-border">
            ...
            <div className="card-content filters-grid">
              ...
              <div className="filter-actions">
                <button onClick={resetAllFilters} className="pixel-btn reset-filters-btn">
                  RESET FILTERS
                </button>
              </div>
            </div>
            ...
            <div className="card-content quest-list-scroll">
              <div className="quest-list">
                {filteredQuests.map(q => (
                  <div 
                    key={q.id} 
                    className="quest-item pixel-border cleared"
                  >
                    <div className="quest-main-content">
                      <div className="quest-row-1">
                        <span className="quest-check-icon" style={{ color: 'var(--color-primary)', marginRight: '8px', fontFamily: 'Press Start 2P', fontSize: '10px' }}>✓</span>
                      ...
                      <div className="quest-row-2">
                        <span className={`badge difficulty diff-${q.difficulty.toLowerCase()}`}>
                          {q.difficulty.toUpperCase()}
                        </span>
                        {q.topics && q.topics.map(topic => (
                          <span key={topic} className="badge category">{topic}</span>
                        ))}
                      </div>
                    </div>
                ...
                {quests.length === 0 && (
                  <div className="no-results">
                    ...
                    {user?.leetcode?.username ? (
                      <>
                        NO CLEARED QUESTS YET.<br />
                        Sync your LeetCode profile to discover your recent Accepted solutions.
                      </>
                    ) : (
                      <>
                        CONNECT YOUR LEETCODE PROFILE<br />
                        Link a public username to begin tracking verified clears.
                      </>
                    )}
                  </div>
                )}
                ...
                {quests.length > 0 && filteredQuests.length === 0 && (
                  <div className="no-results">
                    <span>🔍</span>
                    NO QUESTS MATCH THE CURRENT FILTERS.<br />
        ...
        <div className="level-up-overlay">
          <div className="level-up-box pixel-border">
            ...
            <div className="level-up-info">
              ...
              <p className="level-up-rank">{getPlayerRankName(levelUpData.level)}</p>
            </div>
            <button 
